...
def compare_metrics(
    ...
    if champ_recall - chall_recall > RECALL_DELTA_MAX:
        reasons.append(
            f"Recall caiu demais: {chall_recall:.3f} vs {champ_recall:.3f} (delta > {RECALL_DELTA_MAX})"
        )

    if champ_precision - chall_precision > PRECISION_DELTA_MAX:
        reasons.append(
            f"Precision caiu demais: {chall_precision:.3f} vs {champ_precision:.3f}"
        )
...
def run_training(data_path: Path, artifacts_dir: Path, version: str) -> Dict[str, Any]:
    """
...
def main():
    ...
    parser.add_argument(
        "--new_version", "-v", required=True, help="Versão do challenger (ex: v1.2.0)"
    )
    parser.add_argument(
        "--data", "-d", required=True, help="Caminho dos dados de treino"
    )
    parser.add_argument(
        "--registry", "-r", default="models/registry", help="Diretório do registry"
    )
    parser.add_argument(
        "--artifacts", "-a", default="artifacts", help="Diretório de artefatos"
    )
    parser.add_argument("--baseline", "-b", help="Diretório do baseline")
    parser.add_argument(
        "--dry_run", action="store_true", help="Apenas compara, não promove"
    )
    parser.add_argument(
        "--force", action="store_true", help="Promove mesmo se guardrails falharem"
    )
