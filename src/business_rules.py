...
class INDECalculator:
    ...
    def compute_inde(
        ...
        for indicator, weight in weights.items():
            col_name = (
                f"{indicator}{indicator_suffix}" if indicator_suffix else indicator
            )
...
class BusinessRulesValidator:
    ...
    def validate_ian(
        ...
        # Compara
        valid_mask = (ian_expected.isna() & ian_actual.isna()) | (
    ...
    def validate_ida(
        ...
        ing_values = (
            df[ing_col] if ing_col in df.columns else pd.Series([np.nan] * len(df))
        )
        ...
        # Compara
        valid_mask = (ida_expected.isna() & ida_actual.isna()) | (
        ...
        return ValidationResult(
            ...
            details={
                ...
                "ing_missing_rate": ing_values.isna().mean()
    ...
    def validate_inde(
        ...
        inde_expected = df.apply(
            lambda row: self.calculator.compute_inde(
                row, indicator_suffix=indicator_suffix
            ),
            axis=1,
        )
        ...
        # Compara
        valid_mask = (inde_expected.isna() & inde_actual.isna()) | (
