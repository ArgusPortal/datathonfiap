...
def get_feature_list(df: pd.DataFrame, exclude_patterns: List[str] = None) -> List[str]:
    ...
    features = [
        c for c in df.columns if not any(p in c.lower() for p in exclude_patterns)
    ]
...
def normalize_fase_columns(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for col in fase_cols:
        df[col] = df[col].apply(
            lambda x: FASE_MAP.get(str(x).strip().lower(), np.nan)
            if pd.notna(x)
            else np.nan
        )
...
def create_missing_indicators(
    df: pd.DataFrame, columns: List[str] = None, min_missing_pct: float = 0.05
) -> pd.DataFrame:
    """
    ...
    for prefix in columns:
        ...
        matching = [
            c
            for c in df.columns
            if c.lower().startswith(prefix.lower()) and not c.endswith("_missing")
        ]
...
def create_tenure_feature(df: pd.DataFrame, reference_year: int = 2023) -> pd.DataFrame:
    """
    ...
    for prefix in INDICATOR_PREFIXES:
        ...
        if col_22 and col_23:
            if pd.api.types.is_numeric_dtype(
                df[col_22]
            ) and pd.api.types.is_numeric_dtype(df[col_23]):
...
def select_features_by_prefix(
    df: pd.DataFrame, allowed_prefixes: List[str] = None, exclude_cols: List[str] = None
) -> List[str]:
    """
...
def validate_features_for_year(
