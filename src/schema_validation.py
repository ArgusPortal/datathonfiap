...
def validate_input_schema(
    ...
    if mode == "training":
        if TARGET_COL not in received_features:
            ...
            if invalid_target:
                errors.append(
                    f"Target deve ser binário (0/1), encontrado: {invalid_target}"
                )
    ...
    logger.info(
        f"Schema validado com sucesso (mode={mode}, features={len(expected_features)})"
    )
...
def validate_training_data(
    df: pd.DataFrame, signature: Optional[Dict[str, Any]] = None
) -> None:
    """
    ...
    if TARGET_COL in df.columns:
        ...
        if minority_ratio < 0.1:
            logger.warning(
                f"Dataset desbalanceado: classe minoritária = {minority_ratio:.1%}"
            )
