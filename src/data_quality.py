...
class DataQualityChecker:
    ...
    def check_duplicates(self, key_column: str = "ra") -> QualityCheckResult:
        ...
        result = QualityCheckResult(
            ...
            details={
                ...
                "duplicate_keys": duplicates[key_column]
                .unique()
                .tolist()[:10],  # Primeiros 10
            },
        )
    ...
    def check_ranges(self) -> QualityCheckResult:
        ...
        result = QualityCheckResult(
            ...
            if violations
            else "Todos indicadores dentro do range",
    ...
    def check_missing_values(
        self, critical_columns: List[str] = None
    ) -> QualityCheckResult:
        """
        ...
        if high_missing:
            message_parts.append(
                f"Colunas acima de {self.MAX_MISSING_RATIO*100}% missing: {len(high_missing)}"
            )
        ...
        result = QualityCheckResult(
            ...
            if message_parts
            else "Missing values dentro do aceitável",
            details={
    ...
    def check_dtypes(self, expected_numeric: List[str] = None) -> QualityCheckResult:
        ...
        for col_name in expected_numeric:
            ...
            if not np.issubdtype(dtype, np.number):
                type_issues.append(
                    {"column": actual_col, "expected": "numeric", "actual": str(dtype)}
                )
    ...
    def run_all_checks(
        self, critical_columns: List[str] = None, feature_columns: List[str] = None
...
def validate_modeling_dataset(
    ...
    passed_labels, _ = checker_labels.run_all_checks(
        critical_columns=["ra", "em_risco"]
    )
