...
def validate_artifacts(artifacts_dir: Path) -> List[str]:
    ...
    for artifact in REQUIRED_ARTIFACTS:
        ...
        possible_names = [
            ...
            f"model_v1.{base_name}"
            if "." in artifact
            else f"model_{base_name}_v1.json",
        ]
...
def promote_champion(
    version: str, registry_dir: Path, promoted_by: str = "manual"
) -> None:
    """
...
def rollback_to(version: str, registry_dir: Path, reason: str = "") -> None:
    ...
    rollback_file = (
        rollback_dir / f"rollback_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    )
...
def main():
    ...
    register_parser.add_argument(
        "--version", "-v", required=True, help="Versão (ex: v1.1.0)"
    )
    register_parser.add_argument(
    ...
    promote_parser = subparsers.add_parser(
        "promote", help="Promove versão para champion"
    )
    promote_parser.add_argument(
        "--version", "-v", required=True, help="Versão a promover"
    )
    promote_parser.add_argument(
        "--registry", "-r", default="models/registry", help="Diretório do registry"
    )
    ...
    rollback_parser = subparsers.add_parser(
        "rollback", help="Rollback para versão anterior"
    )
    rollback_parser.add_argument(
        "--version", "-v", required=True, help="Versão para rollback"
    )
    rollback_parser.add_argument(
        "--registry", "-r", default="models/registry", help="Diretório do registry"
    )
    ...
    if args.command == "register":
        ...
        if not versions:
            ...
            for v in versions:
                ...
                print(
                    f"{v['version']:<12} {v['status']:<12} {v['created_at'][:19]:<25} {champion_mark}"
                )
