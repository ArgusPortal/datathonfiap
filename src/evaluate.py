...
def calculate_metrics(
    ...
    metrics["true_positives"] = (
        int(cm[1, 1]) if cm.shape[0] > 1 and cm.shape[1] > 1 else 0
    )
...
def calculate_calibration_metrics(
    ...
    return {
        ...
        "calibration_error": float(
            np.mean(
                np.abs(
                    np.array(calibration_data["prob_true"])
                    - np.array(calibration_data["prob_pred"])
                )
            )
        )
        if calibration_data["prob_true"]
        else None,
    }
...
def compare_models(
    results: Dict[str, Dict], primary_metric: str = "recall"
) -> pd.DataFrame:
    """
