...
class InferenceStore:
    ...
    def _sanitize_features(
        self, features: Dict[str, Any], allowed_features: List[str]
    ) -> Dict[str, Any]:
    ...
    def _compute_numeric_stats(
        self, instances: List[Dict[str, Any]], allowed_features: List[str]
    ...
    def _count_missing(
        self, instances: List[Dict[str, Any]], expected_features: List[str]
    ) -> Dict[str, int]:
    ...
    def append_event(
        ...
        clean_instances = [
            self._sanitize_features(inst, expected_features) for inst in instances
        ]
    ...
    def _create_aggregate_event(
        ...
        missing_rate = (
            sum(missing_counts.values()) / total_cells if total_cells > 0 else 0
        )
        ...
        return {
            ...
            "numeric_means": json.dumps(
                {k: round(v["mean"], 4) for k, v in numeric_stats.items()}
            ),
            "numeric_mins": json.dumps(
                {k: round(v["min"], 4) for k, v in numeric_stats.items()}
            ),
            "numeric_maxs": json.dumps(
                {k: round(v["max"], 4) for k, v in numeric_stats.items()}
            ),
