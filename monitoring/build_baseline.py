...
def build_feature_profile(
    df: pd.DataFrame, feature_list: List[str], signature: Dict[str, str]
) -> Dict[str, Any]:
...
def build_baseline(
    ...
    if target_col in df.columns:
        ...
        logger.info(
            f"Built score profile from target column with positive rate: {score_profile.get('positive_rate', 0)}"
        )
    else:
        # Create minimal profile
        logger.warning(
            f"Target column '{target_col}' not found, creating minimal score profile"
        )
        score_profile = {
            "count": len(df),
            "mean": 0.5,
...
def main():
    parser = argparse.ArgumentParser(
        description="Build baseline profile for drift detection"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--signature",
        type=str,
        default="artifacts/model_signature_v1.json",
        help="Path to model signature",
    )
    parser.add_argument(
        "--metadata",
        type=str,
        default="artifacts/model_metadata_v1.json",
        help="Path to model metadata",
    )
    parser.add_argument(
        "--source",
        type=str,
        default="data/processed/modeling_dataset.parquet",
        help="Path to training data",
    )
    parser.add_argument(
        "--output", type=str, default="monitoring/baseline", help="Output directory"
    )
