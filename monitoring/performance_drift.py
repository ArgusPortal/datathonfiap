...
def analyze_performance(
    inference_df: pd.DataFrame, labels_df: pd.DataFrame, window_days: int = 30
) -> Dict[str, Any]:
    """
    ...
    merged = inference_df.merge(
        labels_df[["request_id", "label"]], on="request_id", how="inner"
    )
    ...
    y_pred = (
        merged["risk_label"].values
        if "risk_label" in merged.columns
        else (merged["risk_score"] >= 0.5).astype(int).values
    )
    y_proba = (
        merged["risk_score"].values
    ...
    if metrics.get("precision", 1.0) < PRECISION_ALERT:
        alerts.append(
            f"Precision baixo: {metrics['precision']:.3f} < {PRECISION_ALERT}"
        )
...
def main():
    ...
    parser.add_argument(
        ...
        help="Diretório do inference store",
    )
    parser.add_argument(
        "--labels",
        "-l",
        default="monitoring/labels_store.jsonl",
        help="Arquivo de labels",
    )
    parser.add_argument("--window", "-w", type=int, default=30, help="Janela em dias")
    parser.add_argument(
        "--output", "-o", default="monitoring/reports", help="Diretório de saída"
    )
    ...
    # Carrega dados
    inference_df = load_inference_store(
        Path(args.inference_store), start_date, end_date
    )
