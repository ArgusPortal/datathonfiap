...
def compute_psi(baseline_freq: List[float], current_freq: List[float]) -> float:
    """
...
def compute_numeric_psi(
    baseline_profile: Dict[str, Any], current_values: List[float], n_bins: int = 10
...
def compute_categorical_psi(
    baseline_profile: Dict[str, Any], current_values: List[str]
) -> Tuple[float, Dict[str, Any]]:
    ...
    for cat in categories:
        if cat == "__other__":
            other_count = sum(
                current_counts.get(c, 0)
                for c in current_counts.index
...
def analyze_drift(
    ...
    for feature, profile in feature_profiles.items():
        ...
        current_missing = (
            1 - len(current_vals) / len(inference_events) if inference_events else 0
        )
        missing_delta = current_missing - baseline_missing

        status = get_status(
            psi, thresholds["feature_psi_warn"], thresholds["feature_psi_alert"]
        )
        ...
        if abs(missing_delta) >= thresholds["missing_rate_delta_alert"]:
            ...
            abs(missing_delta) >= thresholds["missing_rate_delta_warn"]
            and status == "green"
        ):
            status = "yellow"
    ...
    if score_values:
        ...
        score_status = get_status(
            score_psi, thresholds["score_psi_warn"], thresholds["score_psi_alert"]
        )
    else:
        score_psi = 0.0
    ...
    if n_red > 0 or score_status == "red":
        ...
        n_yellow / n_features > thresholds["yellow_feature_pct"]
        if n_features > 0
        else False
    ):
...
def load_inference_events(
    store_dir: Path, start_date: datetime, end_date: datetime
) -> List[Dict[str, Any]]:
    ...
    for ext in ["parquet", "csv"]:
        ...
        for file_path in files:
            ...
            if (
                file_date.date() < start_date.date()
                or file_date.date() > end_date.date()
            ):
                continue
...
def generate_html_report(drift_metrics: Dict[str, Any], output_path: Path) -> None:
    ...
    sorted_features = sorted(
        feature_drift.items(), key=lambda x: x[1].get("psi", 0), reverse=True
    )
...
def main():
    ...
    parser.add_argument(
        "--model_version", type=str, default="v1.1.0", help="Model version"
    )
    parser.add_argument(
        "--baseline_dir",
        type=str,
        default="monitoring/baseline",
        help="Baseline directory",
    )
    parser.add_argument(
        "--inference_store",
        type=str,
        default="monitoring/inference_store",
        help="Inference store directory",
    )
    parser.add_argument(
        "--output_dir", type=str, default="monitoring/reports", help="Output directory"
    )
    parser.add_argument(
        "--last_n_days", type=int, default=7, help="Number of days to analyze"
    )
