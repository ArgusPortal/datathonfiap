...
def resolve_model_paths() -> Tuple[Path, Path, Path]:
    ...
    if MODEL_VERSION == "champion":
        ...
        if not champion_file.exists():
            logger.warning(
                f"champion.json não encontrado em {registry_path}, usando paths diretos"
            )
            return MODEL_PATH, METADATA_PATH, SIGNATURE_PATH
    ...
    if not version_dir.exists():
        logger.warning(
            f"Versão {version} não encontrada em {registry_path}, usando paths diretos"
        )
        return MODEL_PATH, METADATA_PATH, SIGNATURE_PATH
...
class ModelManager:
    ...
    def load(
        ...
        self._loaded = True
        logger.info(
            f"ModelManager inicializado: v{self.version}, threshold={self._threshold:.4f}"
        )
