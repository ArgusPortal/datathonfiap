...
class TestRegistryFunctions:
    ...
    def temp_artifacts(self, tmp_path):
        ...
        # Signature
        signature = {"input_schema": {"f1": "float", "f2": "int"}}
    ...
    def test_register_model(self, temp_registry, temp_artifacts):
        ...
        version_dir = register_model(
            version="v1.0.0", artifacts_dir=temp_artifacts, registry_dir=temp_registry
        )
    ...
    def test_rollback(self, temp_registry, temp_artifacts):
        ...
        # Registrar v1.1.0
        register_model(
            version="v1.1.0", artifacts_dir=temp_artifacts, registry_dir=temp_registry
        )
    ...
    def test_list_versions(self, temp_registry, temp_artifacts):
        ...
        for v in ["v1.0.0", "v1.1.0", "v2.0.0"]:
            register_model(
                version=v, artifacts_dir=temp_artifacts, registry_dir=temp_registry
            )
...
class TestRegistryCLI:
    ...
    def test_cli_list_empty(self, tmp_path):
        ...
        with patch.object(
            sys, "argv", ["registry", "list", "--registry", str(registry)]
        ):
...
class TestRegistryEdgeCases:
    ...
    def test_missing_required_artifacts(self, tmp_path):
        ...
        with pytest.raises(ValueError) as exc_info:
            register_model(
                version="v1.0.0", artifacts_dir=artifacts, registry_dir=registry
            )

        assert (
            "obrigatórios" in str(exc_info.value).lower()
            or "faltando" in str(exc_info.value).lower()
        )
