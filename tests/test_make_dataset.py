...
class TestFixExcelDateAsNumber:
    ...
    def test_out_of_range_returns_none(self):
        ...
        assert (
            fix_excel_date_as_number("1900-01-02") is None
        )  # Dia 2 = idade 2 (inválido)
...
class TestNormalizeInstituicao:
    ...
    def test_privada_apadrinhamento(self):
        ...
        assert (
            normalize_instituicao("Privada - Programa de Apadrinhamento")
            == "Privada_Apadrinhamento"
        )
        assert (
            normalize_instituicao("Privada - Programa de apadrinhamento")
            == "Privada_Apadrinhamento"
    ...
    def test_privada_bolsa(self):
        ...
        assert (
            normalize_instituicao("Privada *Parcerias com Bolsa 100%")
            == "Privada_Bolsa"
        )
        assert (
            normalize_instituicao("Privada - Pagamento por *Empresa Parceira")
            == "Privada_Bolsa"
        )
...
class TestAllowedFeatureColumns:
    ...
    def test_indicadores_in_allowed(self):
        ...
        for ind in indicadores:
            assert (
                ind in ALLOWED_FEATURE_COLUMNS
            ), f"{ind} não está em ALLOWED_FEATURE_COLUMNS"
